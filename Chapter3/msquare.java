...
public class msquare {
	...
	public static String solveMsquare(int[][] targetSheet) {
		...
		while (bfsQueue.size()!=0) {
			...
			/*if (counter==0) {
				for (int i=0; i<8; i++) {
					System.out.print(sheetA[i/4][i%4] + " ");
				}
				System.out.println();
			}*/
			...
			/*if (counter==0) {
				for (int i=0; i<8; i++) {
					System.out.print(sheetB[i/4][i%4] + " ");
				}
				System.out.println();
			}*/
			...
			/*if (counter==0) {
				for (int i=0; i<8; i++) {
					System.out.print(sheetC[i/4][i%4] + " ");
				}
				System.out.println();
			}*/

			//if (!seenNodes.contains(sheetA)) {
			if (!seen.contains(concat(sheetA))) {
				Node nodeA = m.new Node(sheetA, sequenceA);
				bfsQueue.offer(nodeA); //seenNodes.add(nodeA);
			}
			//if (!seenNodes.contains(sheetB)) {
			if (!seen.contains(concat(sheetB))) {
				Node nodeB = m.new Node(sheetB, sequenceB);
				bfsQueue.offer(nodeB); //seenNodes.add(nodeB);
			}
			//if (!seenNodes.contains(sheetC)) {
			if (!seen.contains(concat(sheetC))) {
				Node nodeC = m.new Node(sheetC, sequenceC);
				bfsQueue.offer(nodeC); //seenNodes.add(nodeC);
			}
		}
	...
	public static void main(String[] args) throws IOException {
		...
		//int[][] targetSheet = {{2,6,8,4},{1,3,7,5}};
